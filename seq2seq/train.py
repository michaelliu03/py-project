...
from seq2seq import seq2seq_model_utils, data_utils
...
def setup_workpath(workspace):
  ...
  # training data
  if not os.path.exists("%s/short_nlg.in" % data_dir):
    n = 0
    f_zip   = gzip.open("%s/train/short_nlg.txt.gz" % data_dir, 'rt')
    f_train = open("%s/short_nlg.in" % data_dir, 'w')
    f_dev   = open("%s/short_nlg_test.in" % data_dir, 'w')
    for line in f_zip:
      f_train.write(line)
      if n < 10000: 
        f_dev.write(line)
